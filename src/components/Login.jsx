...
function ProtectedRoute({ children }) {
  ...
  return localStorage.getItem('user') ? children : null;
}
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      localStorage.setItem('user', JSON.stringify({
        uid: user.uid,
        email: user.email,
        role: formData.role, // Store the selected role
      }));
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      const user = result.user;
       // For Google Sign-in, you might need to fetch/determine the role.
      //  For this example, I'm using the form's role, but you might have a different way to determine it
      localStorage.setItem('user', JSON.stringify({
        uid: user.uid,
        email: user.email,
        role: formData.role || 'teacher', //  default role
      }));
      redirectToDashboard(formData.role || 'teacher');
    } catch (error) {
      setError(error.message);
